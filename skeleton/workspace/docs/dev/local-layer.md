...
| Что | Где | Зачем |
|---|---|---|
| Инструкции агенту | `AGENTS.md` корня (весь слой личный) и `<сервис>/CLAUDE.local.md` | корень — карта и правила; в сервисе — переопределения командных инструкций (стек, отличия команд, грабли) |
...
Требование к репо сервисов: `CLAUDE.local.md`, `*.local*`, `.claude/settings.local.json` — игнорируются
(их `.gitignore` или, не трогая командные файлы, `<repo>/.git/info/exclude`; проверка: `workspace/bin/check/gitignore.sh`).
...
if echo "$cmd" | grep -Eq '(^|[;&| ])make( |$)' && ! echo "$cmd" | grep -q 'Makefile.local'; then
  echo '{"decision":"block","reason":"Используй make -f Makefile.local <target> (см. CLAUDE.local.md сервиса)"}'; exit 0
fi
exit 0
```
