...
Ты работаешь **из корня проекта** `bookly/`. Это git-репозиторий «слоя агента»: здесь карта, правила, знания и
трекер. Репозитории сервисов лежат рядом как обычные папки и **в этот репо не входят** (`.gitignore` — белый список).
...
> Если существует `CLAUDE.local.md` в корне — его инструкции по **запуску окружения, кредам и личным ограничениям
> имеют приоритет** над этим файлом и над `AGENTS.md` сервисов. Он личный и не в git.
